    ...
    <section class="mt-6 !overflow-hidden lg:mt-10">
      <div class="container">
        ...
        <div class="relative grid w-full grid-cols-1 gap-3 p-4 mx-auto mt-3 border rounded-lg md:w-3/4 bg-colorprimarywhite drop-shadow-xl">
          @foreach ($data as $d)
            @if ($d->is_active)  
              <h4 class="text-2xl font-semibold text-center text-colorprimarytosca font-outfit">{{ $d->judul_informasi }}</h4>
              <p class="text-lg font-semibold leading-7 tracking-tight text-colorprimaryblack font-outfit">
                {!! $d->isi_informasi !!}
              </p>
            @endif
          @endforeach
