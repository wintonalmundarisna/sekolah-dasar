<x-filament-panels::page>
        ...
        <div class="flex items-center gap-4 border-t py-6">
            @if ($this->getCurrentTheme() instanceof \Hasnayeen\Themes\Contracts\HasChangeableColor)
                ...
                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <label for="custom">Warna Manual</label>
                </div>
            @else
                <p class="text-gray-700 dark:text-gray-400">Pilih tema yang sudah disediakan</p>
            @endif
        </div>
    </section>
    ...
    <section class="">
        <header class="flex items-center gap-x-3 overflow-hidden py-4">
            <div class="grid flex-1 gap-y-1">
                <h3 class="fi-section-header-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
                    Pilih Tema Yang Sudah Disediakan
                </h3>
        
                <p class="fi-section-header-description text-sm text-gray-500 dark:text-gray-400">
                    Tema bisa kamu ganti kapanpun kamu mau
                </p>
            </div>
        </header>
            ...
            @foreach ($this->getThemes() as $name => $theme)
                ...
                <x-filament::section>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            @if ($supportColorChange)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('Warna dasar bisa diubah') }}',
                                        theme: $store.theme,
                                    }"
                            ...
                            @if (! $noLightMode)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('Mendukung mode terang') }}',
                                        theme: $store.theme,
                                    }"
                                    class="bg-primary-200 flex items-center justify-center p-1 rounded-full">
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            ...
                            @if (! $noDarkMode)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('Mendukung mode gelap') }}',
                                        theme: $store.theme,
                                    }"
                                    class="bg-primary-200 flex items-center justify-center p-1 rounded-full">
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            ...
                            @if ($this->getCurrentTheme()->getName() === $name)
                                <span
                                    x-data="{}"
                                    x-tooltip="{
                                        content: '{{ __('Yang anda gunakan sekarang') }}',
                                        theme: $store.theme,
                                    }"
                                    class="bg-primary-200 flex items-center justify-center p-1 rounded-full">
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            @if ($noLightMode)
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('Tidak memiliki tema terang') }}</h3>
                            @else
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('Tema terang') }}</h3>
                        ...
                        <div>
                            @if ($noDarkMode)
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('Tidak memiliki tema gelap') }}</h3>
                            @else
                                <h3 class="text-sm font-semibold text-gray-600 pb-4">{{ __('Tema gelap') }}</h3>
